...
player_selections_long <- player_selections_initial |> 
  ...
  group_by(id) |> 
  mutate(
    completion_percentage_diff = c(head(completion_percentage, 1), diff(completion_percentage)),
    ownership_diff = c(head(ownership, 1), diff(ownership)),
    ownership_adjusted_diff = c(head(ownership_adjusted, 1), diff(ownership_adjusted))
  ) |> 
  ungroup() |> 
  mutate(
