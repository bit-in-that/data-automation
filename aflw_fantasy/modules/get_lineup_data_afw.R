...
get_top_n_lineups <- function(n_teams, session_id, ranking_data = read_parquet("aflw_fantasy/data/processed/ranking_data.parquet")) {
  ranking_data |> 
    ...
    pull(userId) |> 
    map(get_afw_team_raw, round = 1, session_id = session_id) |>
    map(convert_lineup_list_to_df) |> 
    list_rbind() |> 
    transform_multiple_lineups(player_data_by_round) |> 
    left_join(
      ranking_data |> ranking_dataselect(user_id = userId, team_name = teamName, overall_rank = overallRank),
      by = "user_id"
    )
}
...
save_top_10000_lineups <- function(session_id) {
  ...
  top_10000_selections <- top_10000_lineups |> 
    ...
    summarise(
      selections_top_10000 = sum(overall_rank <= 10000),
      selections_top_5000 = sum(overall_rank <= 5000),
      selections_top_2000 = sum(overall_rank <= 2000),
      selections_top_1000 = sum(overall_rank <= 1000),
      selections_top_100 = sum(overall_rank <= 100),
      captain_top_10000 = sum(overall_rank <= 10000 & is_captain),
      captain_top_5000 = sum(overall_rank <= 5000 & is_captain),
      captain_top_2000 = sum(overall_rank <= 2000 & is_captain),
      captain_top_1000 = sum(overall_rank <= 1000 & is_captain),
      captain_top_100 = sum(overall_rank <= 100 & is_captain),
      vice_captain_top_10000 = sum(overall_rank <= 10000 & is_vice_captain),
      vice_captain_top_5000 = sum(overall_rank <= 5000 & is_vice_captain),
      vice_captain_top_2000 = sum(overall_rank <= 2000 & is_vice_captain),
      vice_captain_top_1000 = sum(overall_rank <= 1000 & is_vice_captain),
      vice_captain_top_100 = sum(overall_rank <= 100 & is_vice_captain),
      bench_top_10000 = sum(overall_rank <= 10000 & is_bench),
      bench_top_5000 = sum(overall_rank <= 5000 & is_bench),
      bench_top_2000 = sum(overall_rank <= 2000 & is_bench),
      bench_top_1000 = sum(overall_rank <= 1000 & is_bench),
      bench_top_100 = sum(overall_rank <= 100 & is_bench),
      utility_top_10000 = sum(overall_rank <= 10000 & is_utility),
      utility_top_5000 = sum(overall_rank <= 5000 & is_utility),
      utility_top_2000 = sum(overall_rank <= 2000 & is_utility),
      utility_top_1000 = sum(overall_rank <= 1000 & is_utility),
      utility_top_100 = sum(overall_rank <= 100 & is_utility),
      .groups = "drop"
    )
